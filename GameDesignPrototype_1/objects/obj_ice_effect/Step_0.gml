/// @desc Ice Effect - Apply to nearby enemies
if (lifetime <= 0) {
    instance_destroy();
    exit;
}
...
for (var i = 0; i < ds_list_size(hit_list); i++) {
    ...
    // Apply freeze using existing status system
    enemy.status.freeze_timer = slow_duration;
    enemy.status.frozen = true;
    enemy.status.freeze_slow_mult = slow_amount;
    
    // Visual feedback
    repeat(5) {
        var p = instance_create_depth(enemy.x, enemy.y, -100, obj_particle);
        p.sprite_index = spr_ice_particle;
        p.image_blend = choose(c_aqua, c_blue, c_white);
