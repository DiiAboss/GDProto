...
// COMPONENTS 
//--------------------------------
damage_sys = new DamageComponent(self, 2000); // 100 base HP

// Initialize knockback with all your custom settings
knockback = new KnockbackComponent(0, 0);
knockback.bounce_dampening = 1.1;
knockback.min_bounce_speed = 0;
knockback.min_impact_speed = 3;
knockback.impact_damage_multiplier = 0.1;
knockback.max_impact_damage = 999;
knockback.cooldown_max = 10;
...
// NEW: Holy water chain reaction tracking - Experiemental, needs a universal tracking for more than just holy water
...
/// Called from a main controller object to avoid muiltiple objects calling stepevents
...
if (!knockback.IsActive() && !is_falling && _player_exists) {
...
if (!is_falling && can_fall && knockback.IsActive()) {
    // Only check for pit fall if being knocked back OR moving fast
    var is_being_knocked = knockback.IsActive();
