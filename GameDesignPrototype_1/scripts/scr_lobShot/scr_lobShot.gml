...
function Jump(_self, _spd, _direction, _xStart, _yStart, _targetDistance)
{
    var _dirRad = degtorad(_direction);
    var _outlineX = _xStart + _targetDistance * cos(_dirRad);
    var _outlineY = _yStart + _targetDistance * sin(_dirRad);

    // Measure full distance and progress
    var totalDist = point_distance(_xStart, _yStart, _outlineX, _outlineY);
    var currDist  = point_distance(_xStart, _yStart, _self.x, _self.y);
    var _progress = clamp(currDist / totalDist, 0, 1);

    var _arcHeight = _targetDistance * 0.25; // tune this to taste
