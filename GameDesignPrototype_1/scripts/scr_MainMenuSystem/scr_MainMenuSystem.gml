...
static DrawCharacterSelect = function(_w, _h, _cx, _cy) {
    ...
    for (var i = 0; i < num_chars; i++) {
        ...
        var target_x = _cx;
        var target_y = _cy + 100; // Default: offscreen below
        var target_scale = 0.5;
        var target_alpha = 0.0;
        
        // Check if this character is in one of the 3 visible slots
    ...
    for (var i = 0; i < num_chars; i++) {
        if (i == curr_index) {
            // Save selected for last
            selected_draw_data = i;
            continue;
        }
        
        if (char_positions[i].alpha > 0.01) { // Only draw if visible
    ...
    draw_text(_cx, _h - 40, "[ENTER] Continue  [ESC] Back");
}
...
static HandleCharacterSelect = function(_input, _audio, _mx, _my) {
    ...
    if (_input.FirePress) {
        for (var i = 0; i < num_chars; i++) {
            ...
            if (pos.alpha < 0.1) continue; // Skip invisible cards
    ...
    static DrawUnlocks = function(_w, _h, _cx, _cy) {
        ...
        draw_set_font(fnt_large);
        draw_set_color(c_yellow);
        draw_text(_cx, 60, "UNLOCKS");
        
        draw_set_font(fnt_default);
        draw_set_color(c_white);
        draw_text(_cx, _cy, "Recent Unlocks Will Appear Here");
        draw_text(_cx, _cy + 30, "(Complete runs to unlock new content)");
